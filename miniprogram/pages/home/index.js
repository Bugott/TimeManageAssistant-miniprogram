...
Page({
  ...
  onAddEvent(e) {
    ...
    HomeModel.addEvent(globalEnv.data.userId, eventTitle).then(
      res => {
        this.setData({
          isCreating: false
        })
  ...
  setTimerTips() {
    ...
    switch (timerInfo.timerState) {
      ...
      case TimerState.PAUSE:
        ...
        this.setData({
          ...
          timerEventId: timerInfo.eventId
        })
        break
    ...
    this.setData({
      ...
      timerEventTitle: timerInfo.eventTitle
    })
  },
  ...
  getEventList() {
    HomeModel.getEventList(globalEnv.data.userId).then(
      res => {
        if (!res.result) {
          this.setData({
            eventList: []
        ...
        const formattedData = HomeModel.formatEventList(res.result.data)
        this.setData({
          eventList: formattedData.list,
  ...
  updatePieOption() {
    const data = HomeModel.serializeForChart(this.data.eventList)
    const {
      min,
      max,
      list
